...
const IconComponent: React.FC<IconComponentProps> = ({ iconName, size = 24, color = 'currentColor' }) => {
  ...
  return Icon ? <Icon width={size} height={size} color={color} /> : <LucideIcons.Circle width={size} height={size} color={color} />;
};
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatsCard
          title="Total Revenue"
          value={`$${reports.salesSummary.totalRevenue.toLocaleString()}`}
          icon="DollarSign"
          color="#10B981"
          change="12.5"
        />
        ...
        <StatsCard
          ...
          value={reports.deliveryStats.assigned + reports.deliveryStats.inTransit}
          icon="Truck"
          color="#8B5CF6"
          change="5.1"
        />
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-orange-50 rounded-lg">
            <div className="text-2xl font-bold text-orange-600">{reports.orderStats.pending}</div>
            <div className="text-sm text-orange-700">Pending</div>
          </div>
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{reports.orderStats.processing}</div>
            <div className="text-sm text-blue-700">Processing</div>
          </div>
          <div className="text-center p-4 bg-yellow-50 rounded-lg">
            <div className="text-2xl font-bold text-yellow-600">{reports.orderStats.shipped}</div>
            <div className="text-sm text-yellow-700">Shipped</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{reports.orderStats.delivered}</div>
