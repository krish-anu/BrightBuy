...
export default function ConfirmationItemsRow({ item, disabled = false }: { item: any; disabled?: boolean }) {
    return (
        <ToggleGroupItem
            value={item.value}
            disabled={disabled}
            className={`justify-between cursor-pointer ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
