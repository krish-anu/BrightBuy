...
const updateVariantStock = async (req, res, next) => {
  ...
  try {
    ...
    const [result] = await connection.query(variantQueries.updateStockAtomic, [
      qnt,
      req.params.id,
      qnt,
    ]);
...
const getTotalLowStockVariants = async (req, res, next) => {
  try {
    const qnt = req.query.qnt || 10;
    // console.log(`Getting total low stock variants with quantity: ${qnt}`);
    const [rows] = await pool.query(variantQueries.getTotalLowStock, [qnt]);
    // console.log(rows);

    res.status(200).json({ success: true, data: rows[0].total });
  } catch (error) {
    next(error);
  }
};
