...
const Sidebar = () => {
  ...
  const { getCurrentUserRoutes, getCurrentRoleName, switchRole, availableRoles, currentRole } = useRole();
  const { user, logout } = useAuth();
  const location = useLocation();

  const routes = getCurrentUserRoutes();
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          ...
          >
            <IconComponent iconName={isCollapsed ? "ChevronRight" : "ChevronLeft"} size={16} />
          </button>
        </div>
